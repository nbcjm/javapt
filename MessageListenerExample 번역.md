# **MessageListenerExample** 번역

Constructr 생성자

https://github.com/DV8FromTheWorld/JDA/blob/master/src/examples/java/MessageListenerExample.java#L45
...
public class MessageListenerExample extends ListenerAdapter
{
    ...
    {
        //We construct a builder for a BOT account. If we wanted to use a CLIENT account
        //봇 계정을 위한 빌더를 구축할것이다. 만약 클라이언트 계정을 사용하길 원한다면
        // we would use AccountType.CLIENT
        //AccountType.CLIENT를 
        try
        {
            JDA jda = JDABuilder.createDefault("Your-Token-Goes-Here") //로그인 하는 계정의 토큰
                    .addEventListeners(new MessageListenerExample())   // 이벤트를 처리할 클래스의 인스턴스.
                    .build();
            jda.awaitReady();// Blocking guarantees that JDA will be completely loaded.개런티를 차단함으로서 JDA가 완전히 로딩될 것이다.
            System.out.println("Finished Building JDA!");
        }
        catch (LoginException e)
        {
            //인증에 문제가 생기면 이 예외로 대체될것이다.
            e.printStackTrace();
        }
        catch (InterruptedException e)
        {
            //Due to the fact that awaitReady is a blocking method, one which waits until JDA is fully loaded,
            //awaitReady가 차단 메소드인 점 때문에,
            // the waiting can be interrupted. This is the exception that would fire in that situation.
            //As a note: in this extremely simplified example this will never occur. In fact, this will never occur unless
            // you use awaitReady in a thread that has the possibility of being interrupted (async thread usage and interrupts)
            e.printStackTrace();
